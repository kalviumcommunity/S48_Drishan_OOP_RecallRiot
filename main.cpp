...
class ICard {  // Abstract class (interface) for Card
public:
...
class Card : public ICard {  // Concrete Card class
private:
    char value;
    bool faceUp;
...
class MemoryGame {
private:
    ICard* cards[4][4];  // Use ICard interface for card objects
...
public:
    MemoryGame() : score(0), remainingFlips(30) {
        ...
        for (int i = 0; i < 4; ++i) {
            for (int j = 0; j < 4; ++j) {
                cards[i][j] = new Card(initialCards[index++]);  // Can use other ICard types in future
                totalCards++;
            }
        }
    }
    ...
    void unflipCards(ICard* card1, ICard* card2) {
